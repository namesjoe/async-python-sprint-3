# Запуск проекта
Запускаем сначала сервер $python run_server.py
запускаем первый client как `$python client.py John` -  создается логин в чат с именем John
запускаем второй client как '$python client.py Anna' - создается логин в чат с именем Anna
пишем в оба клиента и наблюдаем как в чат отображатся сообщения от Анны и Джона
чтобы отправить файл надо ввести sendfile в чате и после ввести путь к файлу

Код сервера запускается через `run_server.py`
Клиент запускается через  `client.py`
Тесты функционала доступы в `test_server.py`
